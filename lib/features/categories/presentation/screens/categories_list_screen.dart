...
/// Punto 7: Swipe entre pestañas sincronizado con TabController + PageView.
...
class _CategoriesListScreenState extends ConsumerState<CategoriesListScreen>
  ...
  void initState() {
    super.initState();
    // Inicializar TabController en la pestaña activa del provider
    final initialType = ref.read(categoryTypeFilterProvider);
    final initialIndex = _types.indexOf(initialType).clamp(0, 1);
    ...
    // Cuando el TabController cambia (tap en pestaña), animamos el PageView
    _tabController.addListener(() {
      ...
      ref.read(categoryTypeFilterProvider.notifier).state =
  ...
  Widget build(BuildContext context) {
    ...
    final backgroundColor =
        isDark ? AppColors.backgroundDark : AppColors.backgroundColor;
        ...
        children: [
          ...
          Container(
            margin: const EdgeInsets.fromLTRB(
                AppColors.pagePadding, 8, AppColors.pagePadding, 16),
          ...
          Padding(
            padding: const EdgeInsets.symmetric(
                horizontal: AppColors.pagePadding, vertical: 0),
            child: Container(
              height: 46,
              decoration: BoxDecoration(
                color:
          ...
          // ── PageView con swipe sincronizado ─────────────────────────────
          Expanded(
            child: PageView(
              controller: _pageController,
              // Al hacer swipe, actualizamos el tabController y el provider
              onPageChanged: (index) {
              ...
              children: [
                _buildCategoryPage(context, 'ingreso', isDark),
                _buildCategoryPage(context, 'gasto', isDark),
              ],
            ),
          ),
        ],
  ...
  /// Construye la página (grid) de categorías para un tipo dado
  Widget _buildCategoryPage(
      BuildContext context, String type, bool isDark) {
    final categoriesAsync = type == 'ingreso'
        ? ref.watch(incomeCategoriesProvider)
        : ref.watch(expenseCategoriesProvider);
    final query =
        ref.watch(categorySearchQueryProvider).toLowerCase();
      ...
      data: (categories) {
        ...
        if (filtered.isEmpty) {
          return _buildEmptyState(
              context, isDark, type, isSearch: query.isNotEmpty);
        }
        ...
        return GridView.builder(
          ...
          itemBuilder: (context, index) {
            ...
            return CategoryCard(
              ...
              onDelete: () async {
                final confirm =
                    await _showDeleteDialog(context, category.nombre);
  ...
  Widget _buildEmptyState(BuildContext context, bool isDark, String type,
      {bool isSearch = false}) {
    return Center(
      child: Column(
  ...
  Widget _buildErrorState(
      BuildContext context, String error, bool isDark) {
    return Center(
      child: Column(
        mainAxisAlignment: MainAxisAlignment.center,
        children: [
          ...
          Padding(
            ...
            child: Text(
              ...
              textAlign: TextAlign.center,
              style: GoogleFonts.montserrat(
                  color: Colors.grey, fontSize: AppColors.bodySmall),
            ),
          ),
        ],
      ),
    ...
    try {
      ...
      if (context.mounted) {
        showAppToast(context,
            message: 'Categoría eliminada', type: ToastType.success);
      }
    } catch (e) {
      if (context.mounted) {
        showAppToast(context,
